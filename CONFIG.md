...
| 模板名 | 中文 | 繁殖季节 | 幼崽期 | 孕期 | 体型(公/母) | 攻击力倍率(公/母) | 可骑/可鞍 | 喂食发情 | 备注 |
|--------|------|---------|--------|------|------------|------------------|----------|---------|------|
...
> **喂食发情**：所有已支持物种默认开启了条件性繁衍（`RequireFeeding=true`）。在繁殖季节内，需玩家把对应食物扔到地上被生物吃掉后才会发情。食物列见上表"喂食发情"列。详见上文"条件性繁衍参数"。
>
...
>
> **Cow/Bull 互通说明**：`Cow`(母牛) 和 `Bull`(公牛) 是两个独立模板，通过 `Aliases` 配置互通交配。母牛用 `CubTemplates={"Cow":1,"Bull":1}` 按权重随机生小母牛或小公牛。牛场需同时养母牛群和公牛群。
>
